...
class TestCreateOrderColorSelection:
    ...
    @allure.description('Параметрический тест, проверяющий создание заказа с различным сочетанием цветов "BLACK" и "GREY", и без выбора цвета.'
                        ...
                        'Проверяем, что ответ 201 Created, тело ответа содержит json c track номером заказа и track - число.')
    ...
    def test_create_order_any_color_selection_success_201(self, color):
        ...
        assert 'track' in response.json().keys() and len(str(response.json().get('track'))) != 0, f'Track не получен: {response.json()}'
        assert isinstance(response.json().get('track'), int), f'Track не число: {response.json()}'